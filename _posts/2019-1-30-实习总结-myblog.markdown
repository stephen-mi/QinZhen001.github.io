...
先写下最重要的这个sdk，一开始本来是打算在wepy框架基础上，进行再次封装，做成一个内部使用的框架，但由于框架太散，对业务代码的阅读有一定的影响，而且容易被人修改，和老大讨论后做成了sdk的形式(如同一个黑盒)，减少对业务代码的侵入(不用到处import)。另外，我这边是从零开始写需要频繁迭代更新，导致sdk使用者频繁地去pull，所以最终决定将sdk做成了一个git submodule去管理。同时，将sdk的配置项抽离出来，由外部的配置文件(baseConfig.js, reqConfig.js)决定。一开始写sdk是采取面向对象的思想，但写到后面发现sdk的通用性存在一定的局限性，经老大指导，最终选择采取[IoC依赖倒置模式设计](https://juejin.im/post/5c2c47dcf265da616d544a53)，但考虑到团队的编程习惯，更倾向使用配置文件，所以并没有采用IoC模式的use方法去加载模块，而采用去读配置文件来选择使用什么模块，所以只能算是伪IoC模式。


![enter description here](https://github.com/QinZhen001/QinZhen001.github.io/blob/master/img/in-post/shixi_2.png)


sdk挂载在wepy.$instance.xhwSdk这里 

![enter description here](https://github.com/QinZhen001/QinZhen001.github.io/blob/master/img/in-post/shixi_1.png)
...
* 需要监控上报处理 (请求错误或请求异常时 上报对应的情况 方便监控)
* 部分接口依赖sessionid openid
* 部分请求使用protobuf协议
* **一些特殊请求会请求多次**


设计封装了三种请求xhwRequest(普通请求) xhwRequestWithProto(protobuf协议的请求) xhwGetJson(获取json文件的请求)
...
亮点就是将needSessionId和needOpenId做成配置，请求前会根据这个自动去寻找sessionid和openid
...
**核心思想：维护一个全局队列，每个发请求的方法都返回一个Promise对象，第一个发请求的方法进去队列后，把锁打开，这样后面的发请求方法就不会真的发请求出去，把后面发请求的方法的resolve存到这个这个队列，当第一个请求拿到response后，后面的请求也
resolve这个response，最后再把锁关掉。**
...
**原因是wx.login去拿code 这里会调用多次 而我封装成单例  但只有第一个code有效 有一定延时**


最终解决方案，就是上面的singletonForLogin

将wx.login去拿code 和 拿code去makeSession封装成一个单例，也就是一个全局队列
...
![enter description here](https://github.com/QinZhen001/QinZhen001.github.io/blob/master/img/in-post/shixi_3.png)
...
![enter description here](https://github.com/QinZhen001/QinZhen001.github.io/blob/master/img/in-post/shixi_4.png)
...
![enter description here](https://github.com/QinZhen001/QinZhen001.github.io/blob/master/img/in-post/shixi_5.png)
...
零做后端系统页面也就是vue+element没什么难点，猜字类小程序的难点在于答案在不同的字段时要显示不同的UI，这也就出现了16种可能，还有当跳转到下一题时要有淡入淡出的效果。这里可以提一下做的一些基础组件
...
本质上就是一个image通过动态改变image的src来发送get请求从而达到上报错误
...
因为由它来判断Monitor组件是否已经准备好了，准备好了的状态global.xhw.reportReady是一个全局状态，并不是每一个组件都维护一个这样的状态
