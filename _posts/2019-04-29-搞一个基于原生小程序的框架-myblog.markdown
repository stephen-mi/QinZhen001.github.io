...
## 框架设计
整体思路是用gulp复制src文件夹中的文件到dist文件夹，在个过程中，处理相关文件。如处理js文件，用babel将es7语法转成es6；处理less文件，将less文件转wxss；处理图片文件进行压缩等。
...
在production环境中会压缩wxss，压缩js，压缩图片，去除js的在development环境中js会加上sourcemaps
...
第二次尝试：在babel中使用我们不能使用plugin-transform-strict-mode，失败
